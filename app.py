...
def load_page(page):
    try:
        ...
        print(f"ERROR rendering {page}.html: {e}")
        return f"<h2>Template Error on '{page}'</h2><pre>{e}</pre>", 500
...
def get_assignment(patient_id):
    try:
        if ASSIGNMENTS_FILE.exists():
            ...
            return jsonify(all_assignments.get(patient_id, {"exercises": []}))
        return jsonify({"exercises": []})
    except Exception as e:
        print("Get assignment error:", e)
        return jsonify({"exercises": []})

@app.route("/api/assignments/<patient_id>", methods=["POST"])
def save_assignment(patient_id):
    try:
        data = request.get_json(silent=True)
        if not data:
            return jsonify({"error": "Invalid payload"}), 400
        ...
        if ASSIGNMENTS_FILE.exists():
            with open(ASSIGNMENTS_FILE, "r") as f:
                all_assignments = json.load(f)
        all_assignments[patient_id] = data
        with open(ASSIGNMENTS_FILE, "w") as f:
            json.dump(all_assignments, f, indent=4)
        return jsonify({"success": True})
    except Exception as e:
        print("Save assignment error:", e)
        return jsonify({"error": str(e)}), 500
